...
class nnUNetTrainerV2_S6_D2_W16_FLARE(nnUNetTrainerV2):
    def initialize_network(self):
        ...
        #                 self.network,  # the original model
        #                 {nn.Conv3d},  # a set of layers to dynamically quantize
        #                 dtype=torch.qint8)
        # print('quantization time: ', time.time()-start)

        start = time.time()
        if torch.cuda.is_available():
            self.network.cuda()
        self.network.inference_apply_nonlin = softmax_helper
        print('network to cuda time: ', time.time()-start)
